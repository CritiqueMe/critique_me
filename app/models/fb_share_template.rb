...
class FbShareTemplate < ActiveRecord::Base
  def build_message(tracking_object)
    subs(message, tracking_object)
  end
  ...
  def build_link_display(tracking_object)
    subs(link_display, tracking_object)
  end
  ...
  def build_caption(tracking_object)
    subs(caption, tracking_object)
  end

  def build_description(tracking_object)
    subs(description, tracking_object)
  end

  private

  def subs(str, question)
