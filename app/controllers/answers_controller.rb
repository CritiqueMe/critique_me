class AnswersController < ApplicationController
  ...
  def new
    if request.post?
      ...
      questionnaire = @answer.question.default_question.try(:questionnaire)
      if questionnaire
  ...
  def canned_answer
    if request.post?
      ...
      if params['post_to_timeline'] == '1'
        FbShare.share(@user, q, session[:fb_access_token], [params['chosen_friend']], [cqc.friend_name], nil, 'friends_wall')
